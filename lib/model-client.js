...
const { assertHttpOrHttpsUrl } = require('./url-safety');
...
function normaliseOpenAICompatibleChatUrl(baseUrl = '') {
  ...
  const normalised = raw.endsWith('/chat/completions')
    ? raw
    : raw.replace(/\/$/, '').replace(/\/v1$/, '/v1') + '/chat/completions';
  return assertHttpOrHttpsUrl(normalised, 'Model base URL').toString();
}
