...
const { validateHttpUrl, resolveRedirectUrl, assertSafeResearchUrl } = require('./url-safety');
...
async function scrapeURLFast(targetUrl, redirectCount = 0) {
  ...
  const parsed = await assertSafeResearchUrl(targetUrl);
  const protocol = parsed.protocol === 'https:' ? https : http;

  return new Promise((resolve, reject) => {
    const request = protocol.get(parsed.href, (res) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        res.resume();
        let nextUrl;
        try {
          nextUrl = resolveRedirectUrl(parsed.href, res.headers.location);
        } catch (err) {
          return reject(err);
        }
      ...
      res.on('end', async () => {
        try {
          const findings = await analyseHTMLFast(raw, parsed.href);
          resolve(findings);
        } catch (err) {
          reject(err);
        }
...
async function scrapeRenderedURL(targetUrl, dataDir) {
  const parsed = await assertSafeResearchUrl(targetUrl);
  const { screenshotPath, screenshotUrl } = researchAssetPaths(parsed.href, dataDir);
  ...
  try {
    ...
    await page.route('**/*', async (route) => {
      ...
      try {
        await assertSafeResearchUrl(requestUrl);
